...
namespace Valid_Address
{
    ...
    {
        // First matching pattern wins. These are format checks, not checksums.
        static readonly Dictionary<string, string> RegexDictionary = new Dictionary<string, string>
        {
        ...
        {
            Console.WriteLine("Best Coin Address Validator — regex format check, not checksum validation.");
            Console.WriteLine();
            Console.WriteLine("Usage:");
            Console.WriteLine("  Valid-Address                 Interactive prompt");
            Console.WriteLine("  Valid-Address <address> ...   Check one or more addresses and exit");
            Console.WriteLine("  Valid-Address --help          Show this help");
            Console.WriteLine();
            Console.WriteLine("From the repository root:");
            Console.WriteLine("  dotnet run --project Valid-Address/Valid-Address/Valid-Address.csproj -- <address>");
        }

        static void PrintCheckResult(string rawAddress)
        {
            string address = (rawAddress ?? string.Empty).Trim();
            string matchedCurrency;
            if (TryMatchNetwork(address, out matchedCurrency))
            {
        ...
        static bool TryMatchNetwork(string address, out string matchedCurrency)
        {
